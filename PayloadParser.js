...
0x 00 67 00 FE A1
Channel ID = 0x 00, Type ID = 0x67 → MCU temperature data report
0x FE A1 = -351 × 0.1°C = -35.1°C 
0x 00 BA 6E
o Channel ID = 0x 00, Type ID = 0xBA → battery voltage data report
o 0x 6E = (110 × 0.01 V) + 2.50 V = 3.60 V
...
function parseUplink(device, payload) {
    ...
    // Store battery
    if (decoded.battery_status != null ) {
        var sensor1 = device.endpoints.byAddress("1");

        if (sensor1 != null)
            sensor1.updateVoltageSensorStatus(decoded.battery_status);
        device.updateDeviceBattery({ voltage: decoded.battery_status});

    };

  // Store temperature MCU
    if (decoded.mcu_temperature != null) {
        var sensor2 = device.endpoints.byAddress("2");

        if (sensor2 != null)
            sensor2.updateTemperatureSensorStatus(decoded.mcu_temperature);
